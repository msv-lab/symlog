...
def create_naive_domain_facts(p: Program) -> List[Rule]:
    ...
    pred_symconsts_list_map = create_pred_consts_lst_map(
    ...
    for pred_name in pred_symconsts_list_map.keys():
        for sym_consts, consts in itertools.product(
            pred_symconsts_list_map[pred_name], pred_consts_list_map[pred_name]
        ):
            const_facts.extend(
                [
                    create_fact(
...
def transform_into_meta_program(p: Program) -> Program:
    ...
    def transform_declarations() -> Dict[str, List[str]]:
        def transform_declaration(n: Rule) -> List[Tuple[Any, Any]]:
            if not n.body:
                symbolic_consts_of_n_pred = pred_symconsts_map.get(n.head.name, [])
