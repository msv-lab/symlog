...
import pytest
from sympy import multiset_partitions

DEBUG = False


def extract_pred_symconsts_pair(fact):
    return fact.head.name, [
        arg
        for arg in fact.head.args
        if isinstance(arg, String)
        and arg.value.startswith(common.SYMBOLIC_CONSTANT_PREFIX)
    ]
...
def create_naive_domain_facts(p: Program) -> List[Rule]:
    ...
    for pred_name in pred_sym_consts_list_map.keys():
        for sym_consts, consts in itertools.product(
            pred_sym_consts_list_map[pred_name], pred_consts_list_map[pred_name]
        ):
            const_facts.extend(
                [
                    create_fact(
                        f"{common.DOMAIN_PREDICATE_PREFIX}{sym_const.value}", [const]
                    )
                    for (const, sym_const) in zip(consts, sym_consts)
                ]
            )
...
def transform_into_meta_program(p: Program) -> Program:
    ...
    pred_sym_consts_map = utils.flatten_dict(
        create_pred_consts_lst_map(facts, lambda x: extract_pred_symconsts_pair(x))
    )
    ...
    def binding_vars_of_pred(pred_name: str) -> List[Variable]:
        return [
            ...
            for x in pred_sym_consts_map.get(pred_name, [])
        ]
    ...
    def transform_declarations() -> Dict[str, List[str]]:
            ...
            if not n.body:
                ...
                res = [
                    (
                        ...
                        + [common.DEFAULT_SOUFFLE_TYPE]
                        * len(symbolic_consts_of_n_pred),
                    )
                ...
                res.extend(
                    [
                        (
                            f"{common.DOMAIN_PREDICATE_PREFIX}{x.value}",
                            [common.DEFAULT_SOUFFLE_TYPE],
                        )
                        for x in symbolic_consts_of_n_pred
                        if p.declarations.get(
                            f"{common.DOMAIN_PREDICATE_PREFIX}{x.value}", None
                        )
                        is None
                    ]
                )  # domain declarations
                ...
                return [
                    (
                        n.head.name,
                        p.declarations[n.head.name]
                        + [common.DEFAULT_SOUFFLE_TYPE] * len(binding_vars),
                    )
                ]
        ...
        transformed_declarations = {
            k: v for k, v in itertools.chain(*map(transform_declaration, rules))
        }
        ...
        if isinstance(n, Rule):
            if not n.body:
                ...
                domain_body = [
                    ...
                    for x in pred_sym_consts_map.get(n.head.name, [])
                ]
                ...
                replaced_head = transform(
                    n.head,
                    lambda x: add_binding_vars_to_literal(x, binding_vars)
                ...
                replaced_body = list(
                    map(
                        lambda l: transform(
                            l,
                            lambda x: add_binding_vars_to_literal(
                                x,
                                binding_vars_of_pred(x.name)
                                if x.name in edb_names
                                else binding_vars,
                            )
                            if isinstance(x, Literal)
                            else x,
...
def convert_dict_values_to_sets(my_dict):
    ...
    return {
        ",".join([i.value for i in k]): set([i.value for i in v])
