...
def SymbolicSign(fact):
    return Fact(fact.head.name, fact.head.args, True)
...
def update_program(
    ...
    declarations = ori_program.declarations if declarations is None else declarations
...
def substitute(sources, subs: dict):
    """Substitute the input object(s) with the given subs."""

    sources = deepcopy(sources)
    source_tuple = _polymorph_to_tuple(sources)
    ...
    updated = [
        transform(new_source, lambda x: atom_substitute(x, subs))
        for new_source in source_tuple
    ]
